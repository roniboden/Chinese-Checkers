...
def FourPlayers(players_turns, turn_num, overall_turns, win = False, with_computer=False):
    pygame.init()
    pygame.mixer.init()
    ...
    while game_on:
        ...
        if (winner() == False):
            WriteText('Player ' + str(player_index) + '\'s Turn', col_num * cell_size - 370, lines_num * cell_size - 100,
                      50, col)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if clicked_sprites:
                    ...
                    if matrix[clicked_token[0], clicked_token[1]] == player_index:
                        ...
                        move_sound.play()
                        animation()
                        if player_index == 4 and not winner() and with_computer:  # Check if it's the computer's turn
                            # Generate a list of legal moves for the computer
                            computer_moves = []
                            for i in range(17):
                                for j in range(25):
                                    if matrix[i][j] == 4:  # Check if the token belongs to the computer
                        ...
                        if player_index == 1:
                            ...
                            players_turns.append([first_player_coords, second_player_coords,third_player_coords,
                                                 fourth_player_coords])
            if turn_num < overall_turns:
                button("move forward", 336, 460, 140, 30, button_color, inner_color,
                       lambda: FourPlayers(players_turns, turn_num + 1, overall_turns, win, with_computer))
                rect2 = pygame.draw.rect(screen, button_color, pygame.Rect(330, 454, 152, 42), 6, 20)

            def return_main_action():
                # Log the game conduct
                logging.info(json.dumps({"game_state": {"num_players": 4, "players_turns": players_turns,
                                                        "turn_num": turn_num, "overall_turns": overall_turns,
                                                        "win": win, "with_computer": with_computer}}))
            ...
            def load_game(file_path):
                ...
                game_state = extract_game_state_from_log(log_lines)
                if game_state and len(game_state['players_turns'][0]) == 4:
                    FourPlayers(game_state['players_turns'], 0, game_state['overall_turns'], game_state['win'],
