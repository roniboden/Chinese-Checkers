...
class SixChinese:
    ...
    def add_player(self, index):
        ...
        self.fourth_player = self.players_turns[self.turn_num][3]
        self.fifth_player = self.players_turns[self.turn_num][4]
        self.sixth_player = self.players_turns[self.turn_num][5]
        ...
        if index == 4:
            for i in range(len(self.fourth_player)):
                self.matrix[self.fourth_player[i][0]][self.fourth_player[i][1]] = index
        if index == 5:
            for i in range(len(self.fifth_player)):
                self.matrix[self.fifth_player[i][0]][self.fifth_player[i][1]] = index
        if index == 6:
            for i in range(len(self.sixth_player)):
                self.matrix[self.sixth_player[i][0]][self.sixth_player[i][1]] = index
    ...
    def animation(self, moves=[], clicked_token=None):
        ...
        self.colors = [(240, 230, 230), "red", "yellow", "orange", "green", "purple", "blue"]
    ...
    def winner(self):
        ...
        sixth = True
        for i in range(len(self.first_player)):
            if self.matrix[self.first_player[i][0]][self.first_player[i][1]] != 4:
                fourth = False
                break
        for i in range(len(self.fourth_player)):
            if self.matrix[self.fourth_player[i][0]][self.fourth_player[i][1]] != 1:
                first = False
                break
        for i in range(len(self.second_player)):
            if self.matrix[self.second_player[i][0]][self.second_player[i][1]] != 5:
                fifth = False
                break
        for i in range(len(self.fifth_player)):
            if self.matrix[self.fifth_player[i][0]][self.fifth_player[i][1]] != 2:
                second = False
                break
        for i in range(len(self.third_player)):
            if self.matrix[self.third_player[i][0]][self.third_player[i][1]] != 6:
                sixth = False
                break
        for i in range(len(self.sixth_player)):
            if self.matrix[self.sixth_player[i][0]][self.sixth_player[i][1]] != 3:
                third = False
                break
        ...
        if first:
            ...
            self.write_text('Player 4 had won!', self.col_num * self.cell_size - 370, self.lines_num * self.cell_size - 100, 50, 'green')
            self.winner_sound.play()
            self.win = True
            return True
        elif fifth:
            self.write_text('Player 5 had won!', self.col_num * self.cell_size - 370, self.lines_num * self.cell_size - 100, 50, 'purple')
            self.winner_sound.play()
            self.win = True
            return True
        elif sixth:
            self.write_text('Player 6 had won!', self.col_num * self.cell_size - 370, self.lines_num * self.cell_size - 100, 50, 'blue')
            self.winner_sound.play()
            self.win = True
            return True
        else:
    ...
    def main_loop(self):
        ...
        while game_on:
            ...
            fourth_player_coords = [(i, j) for i in range(17) for j in range(25) if self.matrix[i][j] == 4]
            fifth_player_coords = [(i, j) for i in range(17) for j in range(25) if self.matrix[i][j] == 5]
            sixth_player_coords = [(i, j) for i in range(17) for j in range(25) if self.matrix[i][j] == 6]
            if self.player_index == 1: col = 'red'
            if self.player_index == 2: col = 'yellow'
            if self.player_index == 3: col = 'orange'
            if self.player_index == 4: col = 'green'
            if self.player_index == 5: col = 'purple'
            if self.player_index == 6: col = 'blue'

            if not self.winner():
                self.write_text(f'Player {self.player_index}\'s Turn', self.col_num * self.cell_size - 370, self.lines_num * self.cell_size - 100, 50, col)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and not self.win:
                    ...
                    if clicked_sprites:
                        ...
                        if self.matrix[clicked_token[0], clicked_token[1]] == self.player_index:
                            ...
                            self.player_valid_moves = []
                            screen.fill(pygame.Color("white"))
                            self.player_index = (self.player_index + 1) % 7
                            if self.player_index == 0:
                                self.player_index += 1
                            self.move_sound.play()
                            self.animation()

                            if self.player_index == 6 and not self.winner() and self.with_computer:
                                computer_moves = []
                                for i in range(17):
                                    for j in range(25):
                            ...
                            if self.player_index == 1:
                                ...
                                self.players_turns.append([first_player_coords, second_player_coords,third_player_coords,fourth_player_coords,
                                                           fifth_player_coords,sixth_player_coords])
            pygame.display.update()
            timer.tick(60)

    def move_forward(self):
        sleep = time.sleep(1)
        load_next_turn = SixChinese(self.players_turns, self.turn_num + 1, self.overall_turns, self.win,
                                             self.with_computer)
    ...
    def load_game(self,file_path):
        ...
        if game_state and len(game_state['players_turns'][0]) == 6:
            turn = SixChinese(game_state['players_turns'], 0, game_state['overall_turns'], game_state['win'],
                       game_state['with_computer'])
            turn.main_loop()


    def return_main_action(self):
        logging.info(json.dumps({"game_state": {"num_players": 6, "players_turns": self.players_turns,
