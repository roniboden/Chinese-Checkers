...
class TwoChinese:
    ...
    def animation(self, moves=[], clicked_token=None):
        ...
        self.colors = [(240, 230, 230), "red","green"]
    ...
    def winner(self):
        ...
        second = True
        for i in range(len(self.first_player)):
            if self.matrix[self.first_player[i][0]][self.first_player[i][1]] != 2:
                second = False
                break
        for i in range(len(self.second_player)):
            if self.matrix[self.second_player[i][0]][self.second_player[i][1]] != 1:
                first = False
                break
        ...
        if first:
            ...
            self.write_text('Player 2 had won!', self.col_num * self.cell_size - 370, self.lines_num * self.cell_size - 100, 50, 'green')
            self.winner_sound.play()
            self.win = True
            return True
        else:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and not self.win:
                    ...
                    if clicked_sprites:
                        ...
                        if self.matrix[clicked_token[0], clicked_token[1]] == self.player_index:
                            ...
                            self.player_valid_moves = []
                            screen.fill(pygame.Color("white"))
                            self.player_index = (self.player_index + 1) % 3
                            if self.player_index == 0:
                                self.player_index += 1
                            self.move_sound.play()
                            self.animation()

                            if self.player_index == 2 and not self.winner() and self.with_computer:
                                computer_moves = []
                                for i in range(17):
                                    for j in range(25):
    ...
    def move_forward(self):
        sleep = time.sleep(1)
        load_next_turn = TwoChinese(self.players_turns, self.turn_num + 1, self.overall_turns, self.win,
                                             self.with_computer)
    ...
    def load_game(self,file_path):
        ...
        if game_state and len(game_state['players_turns'][0]) == 2:
            turn = TwoChinese(game_state['players_turns'], 0, game_state['overall_turns'], game_state['win'],
                       game_state['with_computer'])
            turn.main_loop()


    def return_main_action(self):
        logging.info(json.dumps({"game_state": {"num_players": 2, "players_turns": self.players_turns,
